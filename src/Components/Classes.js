...
const Classes = ({skills,attributes}) => {
...
console.log(classCss)
  ...
  return (
    <div>
        ...
        <h1>Classes</h1>
            { classCss && Object.keys(CLASS_LIST).map((o,i)=><li style={!classCss[i]?{listStyle:"none"}:{listStyle:"none",color:"red"}} className= {classCss[i] ? "class-list":"class-list highlighted"}>{o}</li>)}
    </div>
  )
}
